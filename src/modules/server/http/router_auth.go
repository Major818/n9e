...
func login(c *gin.Context) {
	...
	err := func() error {
		...
		if config.Config.Rdb.Auth.Captcha {
...
func authAuthorizeV2(c *gin.Context) {
	resp, err := func() (*authRedirect, error) {
		...
		if !config.Config.Rdb.SSO.Enable {
...
func sendLoginCode(c *gin.Context) {
	...
	msg, err := func() (string, error) {
		...
		if !config.Config.Redis.Local.Enable {
		...
		if config.Config.Rdb.Auth.ExtraMode.Debug {
...
func authSettings(c *gin.Context) {
	renderData(c, struct {
		...
		Sso: config.Config.Rdb.SSO.Enable,
	}, nil)
}
...
func passwordChangedNotify(user *models.User) error {
	...
	return redisc.Write(&dataobj.Message{Tos: []string{user.Email}, Subject: _s("[Notify] Password Changed"), Content: buf.String()}, cron.MAIL_QUEUE_NAME)
}
