...
func taskHostStdout(c *gin.Context) {
	...
	if config.Config.Job.OutputComeFrom == "database" || config.Config.Job.OutputComeFrom == "" {
		obj, err := models.TaskHostGet(id, host)
		renderData(c, obj.Stdout, err)
		return
	}

	if config.Config.Job.RemoteAgtdPort <= 0 || config.Config.Job.RemoteAgtdPort > 65535 {
		renderMessage(c, fmt.Errorf("remotePort[%d] invalid", config.Config.Job.RemoteAgtdPort))
		return
	}

	url := fmt.Sprintf("http://%s:%d/output/%d/stdout.json", host, config.Config.Job.RemoteAgtdPort, id)
	client := &http.Client{
...
func taskHostStderr(c *gin.Context) {
	...
	url := fmt.Sprintf("http://%s:%d/output/%d/stderr.json", host, config.Config.Job.RemoteAgtdPort, id)
	client := &http.Client{
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			return http.ErrUseLastResponse
		},
