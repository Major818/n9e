...
func CallbackConsumer() {
	queue := config.Config.Monapi.Queue.Callback
	for {
...
func NeedCallback(sid int64) bool {
	stra, exists := cache.AlarmStraCache.GetById(sid)
	if !exists {
		return false
	}
...
func PushCallbackEvent(event *models.Event) error {
	callbackQueue := config.Config.Monapi.Queue.Callback
...
func doCallback(event *models.Event) {
	stra, exists := cache.AlarmStraCache.GetById(event.Sid)
	if !exists {
