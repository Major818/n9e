...
import (
	...
	"github.com/Major818/nightingale/v4/src/modules/server/notify"
...
func MergeEvent() {
	mergeCfg := config.Config.Monapi.Merge
	for {
...
func parseMergeEvent(eventMap map[int64][]*models.Event) {
	...
	for _, events := range eventMap {
		...
		if len(alertEvents) > 0 {
			...
			for _, bounds := range slice.SplitN(len(alertEvents), max) {
		...
		if len(recoveryEvents) > 0 {
			...
			for _, bounds := range slice.SplitN(len(recoveryEvents), max) {
