...
func Send(items []*dataobj.JudgeItem) {
	for _, item := range items {
		...
		go ToJudge(cache.HistoryBigMap[pk[0:2]], pk, item, now)
	}
}
...
func ToJudge(historyMap *cache.JudgeItemMap, key string, val *dataobj.JudgeItem, now int64) {
	...
	event := &dataobj.Event{
		...
		Detail:    string(bs),
		Value:     value,
		Partition: JudgeConfig.EventPrefix + "/event/p" + strconv.Itoa(stra.Priority),
		Sid:       stra.Id,
...
func getHash(idx query.IndexData, tag string) string {
	if idx.Nid != "" {
		return str.ToMD5(idx.Nid, idx.Metric, tag)
	}

	return str.ToMD5(idx.Endpoint, idx.Metric, tag)
}
